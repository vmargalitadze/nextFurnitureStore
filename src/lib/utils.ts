...
export function convertToPlainObject<T>(value: T): T {
  ...
  const handleDecimal = (obj: any): any => {
    ...
    if (typeof obj === "object") {
      ...
      for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
          result[key] = handleDecimal(obj[key]);
        }
      }
      return result;
    }
...
// Format Number
const NUMBER_FORMATTER = new Intl.NumberFormat("ka-GE");
