...
function PageContentWrapper() {
  ...
  const t = useTranslations("allPage.results");
  ...
  // Parse multiple categories and brands
  const categories = category ? category.split(',').filter(c => c.trim()) : [];
  const brands = brand ? brand.split(',').filter(b => b.trim()) : [];
  ...
  const handleViewSidebar = () => {
    ...
    // Update URL parameters to persist sidebar state
    ...
    const newUrl = `/list${params.toString() ? `?${params.toString()}` : ''}`;
  ...
  const handleFilterChange = (filters: FilterState) => {
    const filtered = products.filter((product) => {
      ...
      const matchesCategory =
        ...
        filters.selectedCategories.some(cat => product.category === cat);

      // Brand filter
      const matchesBrand =
        filters.selectedBrands.length === 0 ||
        filters.selectedBrands.some(brand => product.brand === brand);

      // Search query filter - use localized title
      const localizedTitle = getLocalizedTitle(product);
      const matchesQuery =
        !query ||
        localizedTitle.toLowerCase().includes(query.toLowerCase());
  ...
  useEffect(() => {
    if (products.length > 0) {
      const filtered = products.filter((product) => {
        ...
        const matchesBrand =
          brands.length === 0 ||
          brands.some(b => product.brand?.toLowerCase() === b.toLowerCase());
  ...
  }, [products, categories, brands, query, locale]);
        ...
        <div className="relative z-20 text-center w-full">
          ...
          {filteredProducts.length > 0 && (
            <p className="text-white mt-2 text-sm">
              ...
              {filteredProducts.length === 1 ? t("productCount") : t("productsCount")} found
            </p>
          )}
        </div>
      </div>
